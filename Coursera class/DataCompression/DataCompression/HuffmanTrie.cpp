...
HuffmanTrie::Node::Node(char symbol, int freq) : symbol(symbol), freq(freq), left(nullptr), right(nullptr)
{
}
...
std::unique_ptr<HuffmanTrie> HuffmanTrie::mergeWithTrie(HuffmanTrie& rhs)
{
	...
	this->release();
	rhs.release();
	return std::unique_ptr<HuffmanTrie>(createNewTrie(commonNode));
}
