...
DijkstraSP::DijkstraSP(const EWDiGraph& graph, int source) : source(source), distances(graph.getVertexCount(), std::numeric_limits<double>::max()),
edges(graph.getVertexCount(), Edge(0, 0, 0.0))
{
   ...
   {
      int v = minPQ.delMin();
      const EWDiGraph::EdgesList& adjacent = graph.getAdjacentVertex(v);
...
DijkstraSP::PathToStack DijkstraSP::pathTo(int v) const
{
   ...
   if (!hasPathTo(v))
      return retStack;
   int i = v;
   for (; i != source; i = edges[i].from())
   {
      retStack.push_back(i);
   }
   retStack.push_back(i);
   return retStack;
}
